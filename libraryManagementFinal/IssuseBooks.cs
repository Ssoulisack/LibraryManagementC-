...
{
    public partial class IssuseBooks : UserControl
    {
        ...
        public void DataBookTitle()
        {
            if (conn.State == ConnectionState.Closed)
            {
                ...
                {
                    conn.Open();
                    string selectData = "SELECT bID, book_title FROM books WHERE status = 'Available' AND date_delete IS NULL";
                    ...
                    cmd = new SqlCommand(selectData, conn);
                        ...
                        bookIssue_bookTitle.ValueMember = "bID";
                }
                catch (Exception ex)
                {
        ...
        private void bookIssue_bookTitle_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (conn.State != ConnectionState.Open)
            {
                if (bookIssue_bookTitle.SelectedValue != null)
                {
                    ...
                    int selectID = Convert.ToInt32(selectedRow["bID"]);
                    try
                    {
                        conn.Open();

                        string selectData = "SELECT * FROM books WHERE bID = @id";
        ...
        //Button searchBook by ID
        private void bookIssue_searchbtn_Click(object sender, EventArgs e)
        {
            ...
            {   
                try
                {
                    ...
                    if(dr.Read())
                    {
                        bookIssue_bookTitle.Text = dr["book_title"].ToString();
                        bookIssue_author.Text = dr["author"].ToString();
